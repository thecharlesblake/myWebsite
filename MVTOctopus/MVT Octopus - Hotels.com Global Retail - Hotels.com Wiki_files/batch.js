...
(h?"removefavourite.action":"addfavourite.action"),k={entityId:c}):"space"==e?(j=Confluence.getContextPath()+"/json/"+(h?"removespacefromfavourites.action":"addspacetofavourites.action"),k={key:c}):console.error("Unsupported type: "+e);f.addClass("hidden");g.removeClass("hidden");AJS.safe.ajax({url:j,type:"POST",data:k}).done(function(a){AJS.debug(a)}).fail(function(a,b){AJS.debug("Error Toggling Favourite: "+b)}).always(function(){g.addClass("hidden");f.parent().find(h?".icon-add-fav":".icon-remove-fav").removeClass("hidden");
delete i[c];AJS.trigger(h?"remove-fav-complete":"add-fav-complete")});return!1}}),b.attr("data-favourites-bound",!0))};AJS.Confluence.Binder.favourites=function(){a(".entity-favourites").each(function(){a(this).attr("data-favourites-bound")||g(a(this),{})})};a.fn.favourites=function(b){a(this).each(function(){var d=a(this);g(d,b)})}})(AJS.$);
} catch (err) {
    if (console && console.log && console.error) {
        console.log("Error running batched script.");
        console.error(err);
